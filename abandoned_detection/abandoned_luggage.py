    ...
    for bag in bags:
        ...
        if count == 0:
            ...
            if idx >=0:   
                ...
                cv2.putText(cur_image,"Bag Owner not found", (bag[0],bag[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255))
            ...
            if owner_lost_flag >= 60:
                ...
                if max_avg < 0:
                    ...
                    cv2.rectangle(cur_image, (persons[idx][0], persons[idx][1]), (persons[idx][2], persons[idx][3]), (0,255,255), 2)
                    cv2.putText(cur_image,"owner", (persons[idx][0],persons[idx][1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255))
                    ...
                    if (len(prev_dq_persons[index_row][index_col]))>0:
                        ...
                        for i in range(0,len(persons)):
                            ...
                            if img_iou > 0.40:
                                ...
                                prev_img_person = prev_image[ person_owner_box[1]:person_owner_box[3], person_owner_box[0]:person_owner_box[2] ]


                                resized_image = cv2.resize(prev_img_person, (cur_img_person.shape[1], cur_img_person.shape[0]))
                                resized_image_1 = cv2.resize(cur_img_person, (cur_img_person.shape[1], cur_img_person.shape[0]))

                                perc = compare_images(resized_image, resized_image_1)

                                if max_perc<perc :
                                    max_perc = perc
                                    min_dist = math.sqrt( ((mid_person_x - mid_per_x)**2) + ((mid_person_y - mid_per_y)**2) )
                                    idx = i
            ...
            if idx >= 0:
                ...
                if 125<= bag_dist <= 300:
                    ...
                    cv2.putText(cur_image,"Owner", (persons[idx][0],persons[idx][1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,69,255))
                elif bag_dist > 300: 
                    cv2.rectangle(cur_image, (bag[0], bag[1]), (bag[2], bag[3]), (0,255,255), 2)
                    cv2.rectangle(cur_image, (persons[idx][0], persons[idx][1]), (persons[idx][2], persons[idx][3]), (0,255,255), 2)
                    cv2.line(cur_image,(bag_cord_x,bag_cord_y),(per_cord_x,per_cord_y),(0,255,255),2)
            ...
            for prev_person in person_list:
                if len(prev_person) == 0:
                    ...
                    for cur_person in persons:
                        ...
                        dist = math.sqrt( ((cur_per_mid_x - prev_per_mid_x)**2) + ((cur_per_mid_y - prev_per_mid_y)**2)  )
                    ...
                    if min_dist < 30:
                        ...
                        if 125<= bag_dist <= 300:
                            cv2.rectangle(cur_image, (per_fin[0], per_fin[1]), (per_fin[2], per_fin[3]), (0,255,255), 2)
                            cv2.rectangle(cur_image, (bag_fin[0], bag_fin[1]), (bag_fin[2], bag_fin[3]), (0,255,255), 2) 
                            cv2.line(cur_image,(bag_x,bag_y),(per_x,per_y),(0,255,255),2)
                            cv2.putText(cur_image,"Unattended", (bag_fin[0],bag_fin[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,69,255))
                            cv2.putText(cur_image,"Owner", (per_fin[0],per_fin[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,69,255))
                        elif bag_dist > 300: 
                            cv2.rectangle(cur_image, (per_fin[0], per_fin[1]), (per_fin[2], per_fin[3]), (0,255,255), 2)
                            cv2.rectangle(cur_image, (bag_fin[0], bag_fin[1]), (bag_fin[2], bag_fin[3]), (0,255,255), 2) 
                            cv2.line(cur_image,(bag_x,bag_y),(per_x,per_y),(0,255,255),2)
                            cv2.putText(cur_image,"Abandoned", (bag_fin[0],bag_fin[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255))
                            cv2.putText(cur_image,"owner", (per_fin[0],per_fin[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255))
