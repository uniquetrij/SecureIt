...
from obj_detection.yolo_api.yolo_tf.yolo_v3 import yolo_v3, load_weights, detections_boxes, non_max_suppression

FLAGS = tf.app.flags.FLAGS

tf.app.flags.DEFINE_string('input_img', '../../../data/images/2.jpg', 'Input image')
tf.app.flags.DEFINE_string('output_img', '2_.jpg', 'Output image')
tf.app.flags.DEFINE_string('class_names', '../data/coco.names', 'File with class names')
tf.app.flags.DEFINE_string('weights_file', '../pretrained/yolo_v3.weights', 'Binary file with detector weights')
...
def draw_boxes(boxes, img, cls_names, detection_size):
    ...
    for cls, bboxs in boxes.items():
        color = tuple(np.random.randint(0, FLAGS.size, 3))
